...
def compute_sensor_statistics(pred, target, num_top_sensors=10, mape_threshold=0.1):
    ...
    for i in range(num_sensors):
        ...
        sensor_errors.append({
            "sensor_id": i,
            "mae": mae(sensor_pred, sensor_target),
            "rmse": rmse(sensor_pred, sensor_target),
            "mape": mape(sensor_pred, sensor_target, threshold=mape_threshold),
            "wmape": wmape(sensor_pred, sensor_target),
        })
...
def format_results(results_dict):
    ...
    lines.append(
        f"Sequence Length: {results_dict['seq_len']} steps "
        f"({results_dict['seq_len'] * 5} minutes)"
    )
    lines.append(
        f"Prediction Length: {results_dict['pred_len']} steps "
        f"({results_dict['pred_len'] * 5} minutes)"
    )
    ...
    if "data_distribution" in results_dict:
        ...
        lines.append(
            f"Near-Zero (<0.1): {dist['near_zero_count']:,} "
            f"({dist['near_zero_percent']:.2f}%)"
        )
        lines.append(
            f"Low Speed (<5):  {dist['low_speed_count']:,} "
            f"({dist['low_speed_percent']:.2f}%)"
        )
        lines.append("")
    ...
    lines.append("-" * 80)
    lines.append("Model Type: ST-Transformer")
    ...
    lines.append(
        ...
        f"{results_dict['inference_time_per_sample']:.4f} seconds"
    )
    lines.append("")
    ...
    lines.append(
        f"MAPE (>0.1):    {results_dict['test_mape']:.2f}%  "
        f"(excludes near-zero values)"
    )
    lines.append(
        f"MAPE (masked):  {results_dict['test_mape_masked']:.2f}%  "
        f"(coverage: {results_dict['test_mape_coverage']:.1f}%)"
    )
    lines.append(
        f"WMAPE:          {results_dict['test_wmape']:.2f}%  "
        f"(recommended for low-speed data)"
    )
...
def evaluate(cfg: dict):
    ...
    print(
        f"  Data: min={data_dist['min']:.2f}, max={data_dist['max']:.2f}, "
    ...
    sensor_stats = compute_sensor_statistics(
        pred,
        tgt,
        num_top_sensors=10,
        mape_threshold=cfg.get("mape_threshold", 0.1),
