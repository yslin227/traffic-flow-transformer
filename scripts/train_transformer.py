...
def train():
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                best_model_state,
                os.path.join(checkpoints_dir, "best_transformer.pt")
            )
        else:
            patience_counter += 1
    ...
    with open(
        os.path.join(results_dir, "transformer_results.txt"),
        "w",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        os.path.join(results_dir, "transformer_results_comprehensive.txt"),
        "w",
        encoding="utf-8",
    ) as f:
